      ...
      <div className="mx-auto max-w-7xl px-6 h-14 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <img
            ...
            decoding="async"
          />
          <span className="text-xl font-semibold tracking-tight bg-gradient-to-r from-primary to-accent bg-clip-text text-transparent">Entra RoleReaper</span>
          <div className="hidden md:flex items-center gap-6 ml-6 text-xs text-muted-foreground">
            <span><span className="font-medium text-foreground/80">Reviewer:</span> {reviewerName || "-"}</span>
            <span><span className="font-medium text-foreground/80">Domain:</span> {tenantDomain || "-"}</span>
          </div>
        </div>
        <div className="flex items-center gap-2">
          {authed && <CacheStatusChip accessToken={accessToken} apiBase={apiBase} />}
