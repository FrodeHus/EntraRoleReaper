...
type UserReview = {
  ...
  operations: { operation: string; requiredPermissions: string[] }[];
};
...
export function ReviewPanel({
  ...
  const sorted = useMemo(() => {
    ...
    copy.sort((a, b) => {
      ...
      if (sortBy === 'ops') {
        ...
        cmp = a.userDisplayName.localeCompare(b.userDisplayName, undefined, { sensitivity: 'base' });
      }
  ...
  return (
    <div className="space-y-4">
            ...
            <div className="flex items-center gap-3 text-sm">
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  value={sortBy}
                  onChange={(e) => { setSortBy(e.target.value as 'ops' | 'name'); setPage(1); }}
                >
                  <option value="ops">Operations</option>
                  <option value="name">User name</option>
                </select>
              </div>
              <button className="px-2 py-1 border rounded" onClick={() => { setSortAsc((s) => !s); setPage(1); }}>
                {sortAsc ? 'Asc' : 'Desc'}
              </button>
              <div className="flex items-center gap-1">
                <span>Page</span>
                <button className="px-2 border rounded" disabled={page === 1} onClick={() => setPage((p) => Math.max(1, p - 1))}>{'<'}</button>
                <span>{page}</span>
                <button className="px-2 border rounded" disabled={(page * pageSize) >= (report?.length ?? 0)} onClick={() => setPage((p) => p + 1)}>{'>'}</button>
              </div>
              <select aria-label="page size" className="border rounded px-2 py-1" value={pageSize} onChange={(e) => { setPageSize(Number(e.target.value)); setPage(1); }}>
                  ...
                  <tr key={r.userId} className="border-t align-top">
                    ...
                    <td className="p-2">
                      <div className="flex items-center gap-2">
                        ...
                        <button className="text-blue-600" onClick={() => setOpenSheetFor(r)}>More details</button>
                      </div>
                    </td>
      ...
      <Sheet open={openSheetFor !== null} onOpenChange={(o) => { if (!o) setOpenSheetFor(null) }}>
            {openSheetFor && (
        <SheetContent side="right">
                <SheetHeader>
                  <SheetTitle>Operations for {openSheetFor.userDisplayName}</SheetTitle>
                </SheetHeader>
                  ...
                  <ul className="list-disc pl-4">
                    {openSheetFor.operations.map((op, idx) => (
                      <li key={idx} className="mb-1">
                        ...
                        <div className="text-gray-600 text-xs">{op.requiredPermissions.join(', ') || 'No mapping'}</div>
                      </li>
                    ))}
                  </ul>
                </div>
